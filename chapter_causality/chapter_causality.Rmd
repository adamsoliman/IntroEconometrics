...
([*Appendix:*](#expcond) Conditional Expectation Review)
...
```{r, echo = FALSE}
star_df <- read.csv("../data_for_tasks/star_data.csv")
...
star_df_k_small <- star_df_complete %>%
  filter(grade == "k") %>% # keep only kindergarten
  filter(star != "regular+aide") %>% # remove regular+aide group
  mutate(smallbinary = (star == "small"), # create binary T/F
         small = ifelse(star == "small", 1, 0)) # alternative
```
...
lm(math ~ small, star_df_k_small) # equivalent if lm(math ~ smallbinary, star_df_k_small) 
```
...
b_0 = mean(star_df_k_small$math[
  star_df_k_small$smallbinary == FALSE])
b_0 <- mean((star_df_k_small %>% # same as above
               filter(smallbinary == FALSE))$math)
b_0
```
]
...
delta = mean((star_df_k_small %>% 
  ...
  mean((star_df_k_small %>% 
               filter(small == 0))$math)
delta # equivalent if use smallbinary and TRUE/FALSE
```
]
...
1. Run the code below and answer the question on each line of code.
```{r, echo = TRUE}
star_df_1_regular <- star_df_complete %>%
  filter(grade == "1") %>% # What does this do?
  filter(star != "regular+aide") %>% # What does this do?
  mutate(regular = (star == "regular")) # What does this do?
```

(Final) Run the following regression, `lm(math ~ regular, data = star_df_1_regular` and interpret each coefficient.

---

# Bonus Task: SDO, ATE and Randomization
...
1. Load the data [here](https://github.com/adamsoliman/IntroEconometrics/blob/master/data_for_tasks/toy_data_2.csv) using `read.csv`. The `group` variable corresponds to whether the individual has been treated or not, `Y0` to the potential outcome if the individual does not receive the treatment $(Y_i^0)$ while `Y1` to the potential outcome if the individual receives the treatment $(Y_i^1)$. Create a new variable containing the observed outcome $(Y_i)$ and the individual treatment effect $(\delta_i)$. Recall $Y_i = D_i \times Y_i^1 + (1 - D_i) \times Y_i^0$, $\delta_i = Y_i^1 - Y_i^0$.
...
# Expectation and Conditional Expectation (continued)
