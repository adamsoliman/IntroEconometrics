...
star_df_k_small <- star_df_complete %>%
  ...
  mutate(small = (star == "small"), # create binary T/F
         smalldummy = ifelse(star == "small", 1, 0)) # alternative
```
...
lm(math ~ small, star_df_k_small) # equivalent if lm(math ~ smalldummy, star_df_k_small) 
```
...
delta = mean((star_df_k_small %>% 
               filter(smalldummy == 1))$math) - 
  mean((star_df_k_small %>% 
               filter(smalldummy == 1))$math)
delta # equivalent if use small and TRUE/FALSE
```
]
