...
.pull-right[
```{r}
promotions %>% group_by(gender, decision) %>% 
  summarize(n = n()) %>% mutate(proportion = n / sum(n))
```

```{r}
promotions_shuffled %>% group_by(gender, decision) %>% 
  summarize(n = n()) %>% mutate(proportion = n / sum(n))
```
]
...
.right-thin[
* $\hat{\theta}$ is *some* test statistic. $f(\hat{\theta}|\theta_0)$ and $f(\hat{\theta}|\theta_A)$ are Null and Alternative distributions. Changing $\alpha$ moves critical value $\hat{\theta}_c$. This example is fully worked out [here](https://scpoecon.github.io/ScPoEconometrics/images/hypothesis.pdf)
]
