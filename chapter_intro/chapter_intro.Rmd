...
```{r}
murders <- read.csv("/Users/adamsoliman/Documents/GitHub/Econometrics-Slides/data_for_tasks/gun_murders.csv")
...
2\. First, download [clemsonFBSfinances.csv](https://github.com/adamsoliman/IntroEconometrics/blob/master/data_for_tasks/clemsonFBSfinances.csv) from Github (click download raw file in the top right of the webpage). Then, in the same script used in the previous task, import it into R in a new object and call it `clemsonfootballdata`. This file contains data on Clemson football finances. It should look something like:
 
```{r}
# this is a comment, which you should always use in your code. Note your path will look slightly different
clemsonfootballdata <- read.csv("../data_for_tasks/clemsonFBSfinances.csv")
```
