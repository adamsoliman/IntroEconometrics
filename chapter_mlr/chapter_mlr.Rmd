...
* Let's go back to Angrist and Lavy's (1999)'s analysis of the effect of class size on student performance.
...
grades <- read.csv("../data_for_tasks/grade5_isl.csv")
...
* That class size was positively ***associated*** with students' scores in maths and reading is intuitively unexpected, and contrasts with the simple results from the *STAR* randomized experiment.
...
* To do so, we have to include both `classize` and `disadvantaged` variables as *regressors* in the regression. As such, we can obtain an estimate of the effect of class size on average math score, ** *purged* of the effect of the disadvantaged variable**.
...
* Recall from a few weeks ago, the **Simple Linear Model** can be written as
...
### Interpretation Review for SLR and MLR
...
How do you interpret the intercept and slope coefficients?
...
How do you interpret the intercept and both slope coefficients?
...
***Omitted variable bias*** occurs when we omit a variable that: (1) affects our outcome variable, and (2) is correlated with our regressor of interest. NOTE: This is different from an omitted category!

Let's denote $y$ our outcome variable, $x$ our regressor of interest, and $z$ our omitted variable.

We could run the following regressions:

1. ***Simple linear model:*** $y = b_0 + b_1x + e$

2. ***Multiple linear model:*** $y = c_0 + c_1x + \color{#d90502}{c_2}z + e$

3. ***Omitted variable on regressor:*** $z = d_0 + \color{#d96502}{d_1}x + e$
...
Therefore, through substitution, we get the following:
...
# Omitted Variable Bias (OVB): The Intuition

Because \(x\) is correlated with the omitted variable \(z\), a change in \(x\) is also associated with a change in \(z\). The simple regression therefore attributes both the relationship of \(x\) with \(y\) and the indirect relationship operating through \(z\) to \(x\) $\implies b_1 = c_1 + \text{OVB}$.
...
What do these two pieces tell us?

* $\color{#d90502}{c_2}$: How is the omitted variable $z$ related to the outcome $y$, holding $x$ fixed?

* $\color{#d96502}{d_1}$: How is the omitted variable $z$ related to our regressor $x$?

--

For omitting $z$ to bias the coefficient on $x$, ***both relationships must exist.***

If $c_2 = 0$ or $d_1 = 0$, then:

$$
\text{OVB} = 0
$$

***Not every omitted variable creates omitted variable bias!***

---

# Omitted Variable Bias (OVB): Which Direction?

Since $b_1 = c_1 + \text{OVB},$ the sign of the OVB tells us whether $b_1$ is ***too high*** or ***too low***.
___

If $c_2$ and $d_1$ have the ***same sign***:
...
then OVB is ***positive*** and:

$$
b_1 > c_1
$$

We call this ***upward bias***, since the SLR coefficient $b_1$ is larger than the true effect $c_1$.

___

If $c_2$ and $d_1$ have ***opposite signs***:
...
then OVB is ***negative*** and:

$$
b_1 < c_1
$$

We call this ***downward bias***, since the SLR coefficient $b_1$ is smaller than the true effect $c_1$.

---

# Omitted Variable Bias (OVB): Be Careful With "Up" and "Down"

***Upward bias*** does not necessarily mean that the estimated coefficient is positive.

It means that $b_1$ is ***more positive*** than $c_1$.
...
$$
\text{OVB} = b_1-c_1 = -2-(-5)=+3
$$

So the estimate is ***upward biased*** even though both coefficients are negative.

Likewise, ***downward bias*** means $b_1$ is more negative than $c_1$.

The key question is always:

$$
\boxed{\text{Is } b_1 \text{ above or below } c_1?}
$$

---

# Omitted Variable Bias (OVB): What Do We Use It For?

Usually, if we actually observed $z$, we could simply include it in our regression.

So why is the OVB formula useful?

Most importantly, it lets us use economic reasoning to predict the ***direction*** of the bias.

Ask two questions:

1. What is the expected sign of $\color{#d90502}{c_2}$?

2. What is the expected sign of $\color{#d96502}{d_1}$?
...
If we observe $z$, we can also calculate the ***magnitude*** of the bias:

$$
\text{OVB} = b_1-c_1 = c_2d_1
$$
...
Imagine we want to estimate the relationship between ***income*** and ***years of education***:

$$
\text{income} = b_0 + b_1\text{education} + e
$$

Suppose we omit ***ability***.

Ask our two questions:

1. Is ability related to income, holding education fixed?

2. Is ability correlated with years of education?
...
Given that $b_1 = c_1 + \text{OVB},$ the simple regression coefficient would be ***upward biased***:
...
# Omitted Variable Bias (OVB): Additional Example

Imagine we want to estimate the same relationship between ***income*** and ***years of education***, but suppose this time, we omit ***work experience*** instead. We then ask our two questions:

1. Is work experience related to income, holding education fixed?

   **Yes.** More work experience tends to increase income. $(+)$

2. Is work experience correlated with years of education?

   **Negatively.** Among people of the same age, more years spent in school generally means fewer years of work experience. $(-)$

Therefore: $\text{OVB} = \color{#d90502}{c_2}(+) \times \color{#d96502}{d_1}(-) = (-)$

Given that $b_1 = c_1 + \text{OVB},$ the simple regression coefficient would be ***downward biased***, or $b_1 < c_1$.

--

Ability: Educated people have more of something that raises wages, so education gets too much credit: upward bias.

Experience: Educated people have less of something that raises wages, so education gets too little credit: downward bias.

**NOTE:** When we say “suppose we omit work experience,” we are implicitly holding the rest of the model fixed and asking about the bias attributable to that particular omission. If both ability and experience are omitted, the overall bias is ambiguous because their effects work in opposite directions.
...
$$
\text{average math score} = c_0 + c_1\text{class size} + \color{#d90502}{c_2}\text{% disadvantaged} + e
$$
...
***Omitted variable on regressor:***

$$
\text{% disadvantaged} = d_0 + \color{#d96502}{d_1}\text{class size} + e
$$
...
$$
\color{#d90502}{c_2} = `r round(reg2$coefficients[3],3)`
$$

A higher percentage of disadvantaged students is associated with ***lower math scores***.

And:

$$
\color{#d96502}{d_1} = `r round(reg3$coefficients[2],3)`
$$

Larger classes are associated with a ***lower percentage of disadvantaged students***.
...
>
c_1 = `r round(reg2$coefficients[2],3)`
$$
...
We can also calculate the exact bias:
...
$$
`r round(reg1$coefficients[2],3)`
=
`r round(reg2$coefficients[2],3)`
+
(`r round(reg2$coefficients[3],3)`)
\times
(`r round(reg3$coefficients[2],3)`)
$$
...
\text{OVB}
=
`r round(reg1$coefficients[2] - reg2$coefficients[2],3)`
$$

The simple regression makes the relationship between class size and math scores look ***more positive*** because class size is also picking up differences in the percentage of disadvantaged students.
...
**Submit your answers to Canvas before the next class.** Work through these tasks on your own time. Be prepared to present your approach and answers.

---

# Task 1: Multiple Regression with Reading Scores

Let's analyse the regression results using **reading** score as the dependent variable. Load the data from [here](https://github.com/adamsoliman/IntroEconometrics/blob/master/data_for_tasks/grade5_isl.csv) as `grades`.
...
1. Load the *STAR* data from [here](https://github.com/adamsoliman/IntroEconometrics/blob/master/data_for_tasks/star_data.csv), using `read.csv`, and assign it to an object called `star_df`. Keep only cases with no `NA`s with the following code:  
