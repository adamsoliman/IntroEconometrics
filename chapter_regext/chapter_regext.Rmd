...
* Since for all $x$, $\exp(x)>0$, the natural log function is only defined for ***strictly positive values***! `r emo::ji("warning")` Put another way, you can only log your variables if they don't take 0 or negative values! Always think about this when taking the log of your dependent or independent variable(s)
...
# An example of interpretation of a log-log model

We estimate the effect of fertility on GDP using the **Gapminder** dataset using 
$$ \log(\text{gdp}) = \beta_0 + \beta_1 \cdot \log(\text{fertility}) + \epsilon $$

--

```{r}
lm(log(gdp) ~ log(fertility), gapminder)
```
--

Approximate interpretation for the slope, which works for small changes: A **1% increase in the fertility rate** is associated with approximately a **2.23% decrease in GDP**, on average.

---

# How would you interpret the slope coefficient on this log-level model? 

$$ \log(\text{gdp}) = \beta_0 + \beta_1 \cdot \text{fertility} + \epsilon $$

```{r}
lm(log(gdp) ~ fertility, gapminder)
```
...
**Submit your answers to Canvas before the next class.** Work through these tasks on your own time — you may use any tools or resources. Be prepared to present your approach and answers.

---

# Task 1: Non-Linear Relationships

1. Load the data [here](https://github.com/adamsoliman/IntroEconometrics/blob/master/data_for_tasks/college_tuition_income.csv) and call the object `college`. This dataset contains information about tuition and estimated incomes of graduates for universities in the US.
...
3. Filter the variable `type` to keep only the values of ("For Profit", "Private", "Public"). Call this object `college_clean`. **Use this object from now on.**
...
6. Regress the logarithm of mid-career pay on the logarithm of out-of-state tuition. Interpret the slope coefficient.
...
Let's go back our [grades](https://github.com/adamsoliman/IntroEconometrics/blob/master/data_for_tasks/grade5_isl.csv) dataset. Please download it from the previous link and read it in using the appropriate function. Below are the estimates we got from regressing average math test scores on the full set of regressors.
