...
# an alternative is summary(lm(math ~ small, star_df))
```
...
1\. Copy the loading and cleaning code from slide 3 and run it. Note you will eventually need the **infer** package if you haven't already installed and loaded it...

2\. Generate the bootstrap distribution of $b_\textrm{small}$ based on 1,000 samples drawn from `star_df`.  You can do so through the following code
```{r, eval=FALSE}
