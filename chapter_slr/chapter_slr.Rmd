...
1. Load the data from [here](https://www.dropbox.com/scl/fi/77u2r8xznh63jg7aisq4f/grade5_isl.csv?rlkey=uclo7ng24so9fct41kcjm8k0l&dl=1) as `grades`. Remember that `grades` is called an object. This is similar to a spreadsheet, and you can have multiple objects loaded in R at the same time.
...
1. Describe the dataset:
  ...
  * Use the `skim` function from the `skimr` package to obtain common summary statistics for the variables `classize`, `avgmath` and `avgverb`. You likely need to install `skimr`, so use `install.packages("skimr")`. Note you only need to install a package once, then remember to load the package every time with `library(skimr)` if you want to use it. (*Hint: use `dplyr` to `select` the variables and then simply pipe (`%>%`) `skim()`.*)
...
* These formulas do not appear from magic. They can be found by solving the minimization of squared errors. The maths can be found [here](https://www.youtube.com/watch?v=Hi5EJnBHFB4) for those who are interested.
...
1. Compute the OLS coefficients $b_0$ and $b_1$ of the previous regression using the formulas on slide 27. (*Hint:* you need to use the `cov`, `var`, and `mean` functions.)
