...
# Which "minimization" criterion should (can) be used? 
...
# **O**rdinary **L**east **S**quares (OLS) Estimation: Graphically
...
For now assume both the dependent variable $(y)$ and the independent variable $(x)$ are numeric, then:
...
### Francis Anscombe (1973) came up with 4 datasets with identical stats...remember to visualize!
...
**GOOD CODING PRACTICES TO ALWAYS START WITH: 1) open a new script and save it, 2) load in any packages, such as with *library(tidyverse)*, and 3) add notes for yourself. Then import the data.**

1. Download the data from [here](https://github.com/adamsoliman/IntroEconometrics/blob/master/data_for_tasks/grade5_isl.csv) and load it into R as `grades`. Remember that `grades` is called an object. This is similar to a spreadsheet, and you can have multiple objects loaded in R at the same time.
...
Load the data as `grades`, then run the following code to aggregate at the class size level:
...
1. Regress average verbal score (dependent variable) on class size (independent variable). Interpret the coefficients.
...
1. Bonus: Compute the OLS coefficients $b_0$ and $b_1$ using the formulas from class. (*Hint:* use the `cov`, `var`, and `mean` functions.)
